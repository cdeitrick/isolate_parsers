...
def parse_gd_row(row: List[str]) -> Union[Mutation, Evidence]:
	...
	for i in other:
		...
		a, _, b = i.partition('=') # Use partition because the 'value' field may contain a '='
		keyword_values[a] = b
...
def parse_annotated_gd_file(content: str) -> List[Union[Mutation, Evidence]]:
	...
	gd_data = map(parse_gd_row, gd_table)
	return list(gd_data)
...
def parse_gd_file(io: Union[str, Path], set_index: bool = True) -> pandas.DataFrame:
	...
	return gd_df
#TODO: Refactor into a class and implement a method to extract 'alt' and 'ref' columns
